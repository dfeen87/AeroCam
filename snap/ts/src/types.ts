...
export interface UnifiedMotionState {
  ...
  homographyWarpMatrix: number[]; // 3x3 array (9 elements)
}
...
export interface MotionEventPayloadMap {
  tilt: { angle: number; direction: "left" | "right" | "up" | "down" };
  spin: { angularSpeed: number; axis: "yaw" | "pitch" | "roll" };
  velocity_spike: { magnitude: number; thresholdExceeded: number };
  trajectory_curve: { curvatureRadius: number; predictedNextPosition: Vector3D };
  gesture: { gesture: HeadGesture; confidence: number };
  stability_change: { stabilityScore: number; hintMessage: string };
}

export type MotionEventListener<T extends MotionEventType> = (
